...
public class AdBorderListDao {
	...
	public ArrayList<BorderInfo> getBorderList(String where, int cpage, int psize, String orderBy){
		...
		try {
			stmt = conn.createStatement();
			String sql = "select bs_num, bs_img1, bs_title, bs_start, bs_end, bs_place1, bs_place2, bs_place3, bs_isview, bs_astatus from t_board_story  "
			+ where + orderBy + 
			...
			while (rs.next()) {
				...
				bi.setBs_astatus(rs.getString("bs_astatus"));
				borderList.add(bi);
			}
		}catch(Exception e) {
